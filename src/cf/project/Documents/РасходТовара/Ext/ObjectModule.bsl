...
// Возвращаемое значение: 
//  ТабличныйДокумент - Сформированный табличный документ.
Процедура Пересчитать(Скидка = 0) Экспорт

	Для каждого ТекСтрокаТовары Из Товары Цикл
		
		Если Скидка <> 0 Тогда      
			ТекСтрокаТовары.Сумма = Документы.РасходТовара.РассчитатьСкидку(ТекСтрокаТовары.Количество, ТекСтрокаТовары.Цена, Скидка);  
		Иначе
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	ВидКарты = КартаЛояльности.ВидКарты;
	
	ДосупноКСписанию = 0;
	
	Если ВидКарты = Перечисления.ВидыКартыЛояльности.Накопительная Тогда     
		
		ПараметрыПрограммыЛояльности = Справочники.ПрограммыЛояльности.ПолучитьПрограммуЛояльности(ВидКарты);
		
		Движения.НакопительныеБаллы.Записывать = Истина;
		Движение = Движения.НакопительныеБаллы.Добавить();   
		Движение.Период = Дата;
		Движение.Регистратор = Ссылка; 
		Движение.КартаЛояльности = КартаЛояльности;
		Если ТипОперации = Перечисления.ТипыОпераций.Начисление Тогда 
			
			Движение.ВидДвижения = ВидДвиженияНакопления.Приход;  
			Движение.Сумма  =  Товары.Итог("Сумма") / 100 * ПараметрыПрограммыЛояльности[0].ПравилаНачисления;
			
		ИначеЕсли ТипОперации = Перечисления.ТипыОпераций.Списание Тогда 
			
			Движение.ВидДвижения = ВидДвиженияНакопления.Расход;  			
			Накопления = ПолучитьНакопления(КартаЛояльности);			
			МожноСписать = Товары.Итог("Сумма") / 100 * ПараметрыПрограммыЛояльности[0].ПравилаСписания;			
			ДосупноКСписанию = Мин(Накопления, МожноСписать);			
			Движение.Сумма  =  ДосупноКСписанию;
			Если ДосупноКСписанию = 0 Тогда
				Движения.НакопительныеБаллы.Записывать = Ложь;	
			КонецЕсли;
	...
	Иначе
		...
		Если Курс = 0 Тогда
			Движение.Сумма = Товары.Итог("Сумма") - ДосупноКСписанию;
		Иначе
			Движение.Сумма = Товары.Итог("Сумма") / Курс - ДосупноКСписанию;
		КонецЕсли;
...
Функция ПолучитьНакопления(КартаЛояльности)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	НакопительныеБаллыОстатки.КартаЛояльности КАК КартаЛояльности,
	|	НакопительныеБаллыОстатки.СуммаОстаток КАК СуммаОстаток
	|ИЗ
	|	РегистрНакопления.НакопительныеБаллы.Остатки(&Дата, КартаЛояльности = &КартаЛояльности) КАК НакопительныеБаллыОстатки";
